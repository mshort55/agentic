...
11. **One controller per Kind.** Avoid designs where a single controller reconciles multiple CRD Kinds. This goes against controller-runtime's design and violates encapsulation and the Single Responsibility Principle. Each Kind should have its own controller with its own reconciler. Multiple controllers can run in the same manager binary.

12. **Validate before reconciling.** The reconciliation loop should follow a standard order: retrieve the CR instance, validate it (check for required fields, valid values, etc.), initialize defaults if needed, then proceed with business logic. Don't attempt to reconcile an invalid resource — set a condition indicating the validation failure and return without requeue.

13. **Keep reconciliation loops short.** Exit the reconciler as soon as you determine you cannot complete at this point. Long-running reconciliations block the work queue slot and make resources appear stuck to users. Requeue frequently with `RequeueAfter` rather than waiting inline. Think of reconciliation like yielding from a coroutine — frequent yields enable more apparent concurrency and better UX.
